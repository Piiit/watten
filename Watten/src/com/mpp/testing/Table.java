...
public class Table {
	...
	public void putCardUpdatePlayer(Card card, Player player) throws Exception {
		...
		try {
			...
			throw new Exception("Player " + player.getName() + " can not put card " + card + ", because " + e.getMessage());
		}
	}
	...
	public void resetTablePlayerListOnly() {
		for(int i = 0; i < 4; i++) {
			playerList[i] = null;
		}
	}

	public void resetTableCardListOnly() {
		for(int i = 0; i < 4; i++) {
			cardList[i] = null;
		}
	}
	...
	private String toString(String valueIfNull, String value) {
	...
	public String toString() {
		...
		output = String.format(
				"%25s%-15s\n%25s%-15s\n", 
				" ", 
				toString("(empty)", playerList[0] == null ? null : playerList[0].getName()), 
				" ", 
				toString("(no card)", cardList[0] == null ? null : cardList[0].toString())
				);
		
		output += String.format(
				"%-15s %-15s%3s%-15s%-15s\n", 
				toString("(empty)", playerList[3] == null ? null : playerList[3].getName()), 
				toString("(no card)", cardList[3] == null ? null : cardList[3].toString()), 
				" ", 
				toString("(no card)", cardList[1] == null ? null : cardList[1].toString()), 
				toString("(empty)", playerList[1] == null ? null : playerList[1].getName())
				);
		
		output += String.format(
				"%25s%-15s\n%25s%-15s\n", 
				" ", 
				toString("(no card)", cardList[2] == null ? null : cardList[2].toString()), 
				" ", 
				toString("(empty)", playerList[2] == null ? null : playerList[2].getName())
				);
